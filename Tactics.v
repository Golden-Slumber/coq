...
Example inversion_ex3 : forall (X : Type) (x y z w : X) (l j : list X),
  ...
  x = y.
Proof.
  intros X x y z w l j  e1 e2. inversion e2. inversion e1. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - intros. destruct m.
    + inversion H.
    + simpl in H. inversion H. rewrite <- plus_n_Sm in H1. symmetry in H1.
      rewrite <- plus_n_Sm in H1. inversion H1. symmetry in H2. apply IHn in H2.
...
Proof.
  ...
  - destruct m as [| m'].
    + intros H. inversion H.
    + intros H. apply f_equal. inversion H. apply IHn' in H1. apply H1. Qed.
...
Proof.
  ...
  - intros n. destruct n.
    ...
    + simpl. intros. inversion H. rewrite H1. apply IHl. apply H1. Qed. 
(** [] *)
...
Proof.
  ...
  - intros. unfold split in H. inversion H. unfold combine. reflexivity.
  - intros. destruct x. simpl in H. destruct (split l). inversion H.
    simpl. apply eq_remove_cons. apply IHl. reflexivity. Qed.
(** intros X Y l.
	induction l as [| x y].
	-	intros l1 l2.
		intros eq.
...
Proof.
  intros f b. 
  destruct (f true) eqn: TF1.
  - destruct (f false) eqn: TF2.
    + destruct b.
    rewrite TF1. rewrite TF1. rewrite TF1. reflexivity.
    rewrite TF2. rewrite TF1. rewrite TF1. reflexivity.
    + destruct b.
    rewrite TF1. rewrite TF1. rewrite TF1. reflexivity.
    rewrite TF2. rewrite TF2. rewrite TF2. reflexivity.
  - destruct (f false) eqn: TF3.
    + destruct b.
    rewrite TF1. rewrite TF3. rewrite TF1. reflexivity.
    rewrite TF3. rewrite TF1. rewrite TF3. reflexivity.
    + destruct b.
    rewrite TF1. rewrite TF3. rewrite TF3. reflexivity.
    rewrite TF3. rewrite TF3. rewrite TF3. reflexivity.
  Qed.
(** [] *)
...
Proof.
  ...
  - intros. destruct l2.
    ...
    + simpl in H. inversion H. apply IHl1 in H1. simpl.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
     ...
     test x = true.
Proof.
  intros X f x l lf eq.
  induction l as [| h t IHt].
  - inversion eq.
  - simpl in eq. destruct (f h) eqn: F.
    + inversion eq. rewrite <- H0. apply F.
    + apply IHt. apply eq. Qed.
(** [] *)
...
Proof.
  ...
  induction l as [| h t IHt].
  - simpl. reflexivity.
  - simpl. unfold existsb'.  simpl. destruct (test h) eqn: TF.
    + simpl. reflexivity.
    + simpl. rewrite IHt. unfold existsb'. reflexivity. Qed.
