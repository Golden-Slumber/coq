...
Definition orb2 (b1 b2 : bool) : bool :=
  match b1,b2 with
  | false, false => false
  | _, _ => true
  end.

Compute (orb2 true false).

Example orb2_test :
  (orb2 true false) = true.
Proof. simpl. reflexivity. Qed. 
...
Compute beq_nat1 0 0.
Compute beq_nat1 0 3.
Compute beq_nat1 2 0.
Compute beq_nat1 2 2.
Compute beq_nat1 2 3.
...
Definition blt_nat (n m : nat) : bool :=
  match n with
  | O => negb(beq_nat n m)
  | S n' => andb(leb n m)(negb(beq_nat n m))
  end.
...
Proof.
  intros b c.
  destruct c as [].
  - rewrite andb_commutative. rewrite orb_commutative. simpl. intros H. rewrite H. reflexivity.
  - rewrite andb_commutative. rewrite orb_commutative. simpl. intros H. rewrite H. reflexivity. Qed.
...
Example test_bin_incr1:             (incr Z) = M Z.
Proof. simpl. reflexivity. Qed.
Example test_bin_incr2:             (incr (M (D (M Z)))) = D (M (M Z)).
Proof. simpl. reflexivity. Qed.
Example test_bin_to_nat1:             (bin_to_nat Z) = 0.
Proof. simpl. reflexivity. Qed.
Example test_bin_to_nat2:             (bin_to_nat (M( D (M Z)))) = 5.
Proof. simpl. reflexivity. Qed.
Example test_bin_to_nat3:             (bin_to_nat (D( M (M Z)))) = 6.
Proof. simpl. reflexivity. Qed.
(* Do not modify the following line: *)
