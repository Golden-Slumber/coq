...
Proof.
  ...
  induction n as [| n' IHn'].
  - simpl. reflexivity.
  - simpl. rewrite plus_comm_assoc. rewrite plus_assoc. rewrite plus_assoc.
           assert (H: m+p = p+m). { apply plus_comm. } rewrite H. 
           rewrite H in IHn'. rewrite IHn'. apply plus_assoc. Qed.
  (* intros n m p. induction n as [| n' IHn'].
  - simpl. reflexivity.
...
Proof.
  ...
  - simpl.  assert (H: S n' = 1 + n'). {reflexivity. }
    rewrite H. rewrite mult_distri_l.
    rewrite mult_1_r. rewrite IHn'. reflexivity. Qed. 
...
Proof.
  intros n. induction n as [| n' IHn'].
  - simpl. reflexivity.
  - rewrite IHn'. simpl. rewrite double_neg. reflexivity. Qed. 
(** [] *)
...
(* destruct is classified discussion, while induction is proof by induction. *)
*)
...
(** Translate your solution for [plus_comm] into an informal proof:*)
 (* 
    ...
    - Next, suppose [n = S n'], where
      ...
      By the the induction hypothesis, we just need to show

        S (m + n') = m + S n',

      which is immediate from the plus_n_Sm.  _Qed_.*)
...
Proof.
  ...
  rewrite (plus_comm m n). reflexivity. Qed.
  (*intros n m p. induction n as [| n' IHn'].
  - simpl. reflexivity.
  - simpl. rewrite IHn'. rewrite plus_n_Sm. reflexivity. Qed.*)
...
Proof.
  ...
  - simpl. rewrite mult_0_r. reflexivity.
  - simpl. rewrite <- IHn'. assert(S n' = 1+n') as H.
    { reflexivity. } rewrite H. rewrite mult_distri_l. rewrite mult_1_r. reflexivity. Qed.
  (*intros m n. induction m as [| m' IHm'].  
  - simpl. rewrite mult_0_r. reflexivity.  
  - simpl. assert (H: S m' = 1 + m'). { reflexivity. }   
    rewrite H. rewrite mult_distri_l.     
    rewrite mult_1_r. rewrite IHm'. reflexivity. Qed. *)
(** [] *)
...
Proof.
  intros b.
  rewrite andb_commutative. simpl. reflexivity. Qed.

(* to solve plus_ble_compat_l*)
Theorem leb_simpl : forall n m : nat,
  leb n (n+m) = true.
Proof.
  intros n m.
  induction n as [| n' IHn'].
...
Proof.
  ...
  induction n as [| n' IHn'].
  - simpl. reflexivity.
  - simpl. replace (n' + (m + p)) with (m + (n' + p)). rewrite -> plus_n_Sm. reflexivity. Qed.
(** [] *)
...
Fixpoint bin_to_nat (b : bin) : nat :=
  ...
  | M b' => (mult 2 (bin_to_nat b')) + 1
  end.

Theorem plus_four_swap : forall m n p q : nat,
 m + n + p + q = m + p + (n + q).
Proof.
  intros m n p q.
  induction m as [| m' IHm'].
  - simpl. rewrite plus_swap. rewrite plus_assoc. reflexivity.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. rewrite <- plus_n_O. rewrite <- plus_n_O. rewrite plus_four_swap.
    rewrite IHb2. reflexivity. Qed.
...
Theorem nat_to_nat : forall n : nat,
