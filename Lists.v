...
Example test_create1:            create 5 = [5;4;3;2;1;0].
...
Example test_sum1:            sum [5;4;3;2;1;0] = 15.
Proof. simpl. reflexivity.  Qed.
...
Definition sum1 : bag -> bag -> bag := app.

Example test_sum11:              count 1 (sum1 [1;2;3] [1;4;1]) = 3.
Proof. simpl. reflexivity. Qed.
...
Fixpoint has_member (v:nat) (s:bag) : bool :=
  match s with
  | nil => false
  | h :: t => if beq_nat v h then true
              else has_member v t
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | h :: t => if beq_nat v h then remove_all v t
...
Theorem beq_nat_n : forall n : nat,
  beq_nat n n = true.
Proof.
  intros n.
...
Theorem add_and_count : forall (n:nat) (s:bag),
  count n (add n s) = 1 + count n s.
Proof.
  intros n s.
  induction s as [| h1 t1 IHt1].
  - simpl. rewrite beq_nat_n. reflexivity.
  - simpl. rewrite beq_nat_n. reflexivity. Qed.
...
Theorem nil_app_right : forall l:natlist,
  l ++ [] = l.
Proof.
  intros l. induction l as [| x1 l1 IHl1].
  - simpl. reflexivity.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite beq_nat_n. rewrite IHt. reflexivity. Qed. 
(** [] *)
...
Theorem count_and_sum: forall (s1 s2 : bag),
  count 0 (sum1 s1 s2) = count 0 s1 + count 0 s2.
Proof.
  intros s1 s2. induction s1 as [| h t IHt].
  - simpl. reflexivity.
  - destruct h.
...
Theorem beq_nat_ref1 : forall x, beq_nat x x = true.
Proof.
...
Proof.
  intros x. destruct x. simpl. rewrite -> beq_nat_ref1. reflexivity. Qed.
(** [] *)
